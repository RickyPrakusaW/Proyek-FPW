...
const ReturAdmin = () => {
  ...
  const buttonReturnClasses = isDarkMode
    ? "bg-green-500 px-4 py-2 text-white rounded-md hover:bg-green-600"
    : "bg-green-300 px-4 py-2 text-black rounded-md hover:bg-green-400";
    ...
    <div className={`min-h-screen flex flex-col ${themeClasses}`}>
      ...
      <div className="flex-1 p-5 mx-auto w-4/5">
        ...
        <table className="w-full text-center border-collapse">
          ...
          <tbody>
            {[...Array(6)].map((_, index) => (
              <tr
                key={index}
                className={`${tableRowClasses} border`}
              >
        ...
        <div className="mt-5 flex justify-end">
          <button className={buttonReturnClasses}>
            Retur Barang
          </button>
        </div>
      </div>
